...
video1 = [
    ...
    ("21 Savage", "/channel/1"),
    "21 Savage & Metro Boomin deliver a visual for their platinum selling record, 'X' featuring Future",
    "https://i.ytimg.com/vi/SpXw0qiy3Wo/hqdefault.jpg?custom=true&w=120&h=90&jpg444=true&jpgq=90&sp=68&sigh=DLoxY9SpDoeNH4L0n58Rgl-_Sog",
...
video2 = [
    ...
    "https://i.ytimg.com/vi/zEyEC34MY1A/hqdefault.jpg?custom=true&w=120&h=90&jpg444=true&jpgq=90&sp=68&sigh=rM9BoXNJo2Gwx6Z_bt08X5s6xWw",
...
video3 = [
    ...
    "https://i.ytimg.com/vi/Fi37_GYRYCs/hqdefault.jpg?custom=true&w=120&h=90&jpg444=true&jpgq=90&sp=68&sigh=qij2LjwL7tKV70TUjNw0pTKY3Jg (4KB)",
    "arthur the dog, arthur, arthur sc featured, arthur dog espn feature, sportscenter feature dog, dog remarkable journey",
    ("Sports", "/category/3")]
...
channel1 = [
    ("21 Savage", "/channel/1"),
    "21 Savage's official channel! Subscribe for all music videos, behind the scenes and tour videos from 21 as they'll drop here first.",
    "Feb 8, 2014",
    "United States",
    "319,281,006",
    "734,993"]

channel1_videos = [("21 Savage & Metro Boomin - X ft Future (Official Music Video)", "/video/1")]
...
channel3 = [
    ("ESPN", "/channel/3"),
    "ESPN on YouTube features the best of shows like SportsCenter, SC6, and First Take, along with the most surprising and inspiring stories in sports. Get all your sports videos, sports information, sports news, sports scores, and sports analysis right here.",
    "Oct 31, 2005",
    "United States",
    "5,369,132",
    "1,657,655"]

channel3_videos = [("A Dog's Remarkable Journey To Find A Home | SC Featured", "/video/3")]
channel3_playlists = [("ESPN Originals", "/playlist/3")]

channel_video_list = [channel1_videos, channel2_videos, channel3_videos]
channel_playlist_list = [channel1_playlists, channel2_playlists, channel3_playlists]
...
category1_videos = [("21 Savage & Metro Boomin - X ft Future (Official Music Video)", "/video/1")]
...
category3_videos = [("A Dog's Remarkable Journey To Find A Home | SC Featured", "/video/3")]
category3_channels = [("ESPN", "/channel/3")]


category_video_list = [category1_videos, category2_videos, category3_videos]
category_channel_list = [category1_channels, category2_channels, category3_channels]

categories = [category1, category2, category3]

    
playlist_headers = ["Title", "Description", "Associated Tags", "Date of Publication", "Item Count"]

playlist1 =  [
    ("21 Savage | Music Videos", "/playlist/1"),
    "Music videos by 21 Savage. Various directors. All music can be found here: http://www.livemixtapes.com/main.php?artist=21+savage", 
    "video, sharing, camera phone, video phone, free, upload",
    "Dec 25, 2016",
    "14"]

playlist1_channel = ("21 Savage", "/channel/1")
playlist1_videos = [("21 Savage & Metro Boomin - X ft Future (Official Music Video)", "/video/1")]
...
playlist3 =  [
    ...
    "From '30 For 30' to 'SC Featured,'' these are ESPN's original videos telling some of the greatest stories that transcend sports.", 
    "arthur the dog, arthur, arthur sc featured, arthur dog espn feature, sportscenter feature dog, dog remarkable journey, arthur journey, dog traveled espn, art",
    "Mar 9 2017",
    "15"]

playlist3_channel = ("ESPN", "/channel/3")
playlist3_videos = [("A Dog's Remarkable Journey To Find A Home | SC Featured","/video/3")]


playlist_channel_list = [playlist1_channel, playlist2_channel, playlist3_channel]
...
@app.route('/channel')
def channel():
    return render_template('model.html', title="Channels", table_headers=channel_headers, data=channels)

@app.route('/category')
def category():
    return render_template('model.html', title="Categories", table_headers=category_headers, data=categories)   
...
def video_instance(num):
    return render_template('video.html', headers=video_headers, video_link=video_links[int(num) - 1], video_data=videos[int(num) - 1])
...
def channel_instance(num):
    ...
    #return render_template('channel.html', headers=channel_headers, channel_video_list=channel_video_list[int(num) - 1], channel_playlist_list=channel_playlist_list[int(num) - 1], channel_data=channels[int(num) - 1])
...
def category_instance(num):
    ...
    # return render_template('category.html', headers=category_headers, category_data=categories[int(num) - 1])
...
def playlist_instance(num):
    ...
    # return render_template('playlist.html', headers=playlist_headers, playlist_data=playlists[int(num) - 1])
